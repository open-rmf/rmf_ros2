...
SCENARIO_METHOD(MockAdapterFixture, "dispense item phase", "[phases]")
{
  ...
  auto rcl_subscription =
    adapter->node()->create_subscription<DispenserRequest>(
  ...
  auto dispenser_request_pub =
    adapter->node()->create_publisher<DispenserRequest>(
  ...
  WHEN("it is started")
  {
    ...
    {
      auto result_pub = ros_node->create_publisher<DispenserResult>(
        DispenserResultTopicName, 10);
      auto state_pub = ros_node->create_publisher<DispenserState>(
